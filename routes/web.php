...
// ============================================================================
// RUTAS WEB DESHABILITADAS - APLICACIÓN FUNCIONA SOLO COMO API
// ============================================================================
// Las siguientes rutas han sido deshabilitadas porque la aplicación ahora
// funciona exclusivamente como API. Solo se mantienen activas las rutas de
// generación de PDFs y documentación Swagger.
// ============================================================================
...
// Swagger Documentation Routes (Preservada - útil para documentación de API)
...
// ============================================================================
// RUTAS DE AUTENTICACIÓN WEB DESHABILITADAS
// ============================================================================
// La autenticación ahora se maneja exclusivamente a través de la API
// usando Sanctum en /api/v1/auth/* endpoints
// ============================================================================
...
// ============================================================================
// RUTAS DE VISTAS WEB DESHABILITADAS (API-ONLY)
// ============================================================================
// Todas las rutas web que retornan vistas han sido deshabilitadas.
// Solo se mantienen activas las rutas de generación de PDFs y descargas.
// Todas estas funcionalidades están disponibles a través de la API REST.
// ============================================================================
...
//     Route::get('/matricula/ficha/{cod_alumno}','edit')->name('matricula.ficha');
    ...
    // PRESERVADO: Rutas de generación de PDF de matrícula
    Route::get('/matricula/ficha/{cod_alumno}/descargar',[App\Http\Controllers\MatriculaController::class, 'downloadFicha'])->name('matricula.ficha.download');
    Route::get('/matricula/ficha/{cod_alumno}/ver',[App\Http\Controllers\MatriculaController::class, 'viewFicha'])->name('matricula.ficha.view');
...
//     Route::post('/entry/destroy/{id}','destroy')->name('entry.destroy');
    ...
    // PRESERVADO: Rutas de impresión y PDF de entradas (abonos)
    Route::get('/entry/print/{id}',[App\Http\Controllers\EntryController::class, 'print'])->name('entry.print');
    Route::get('/entry/pdf/{id}',[App\Http\Controllers\EntryController::class, 'ViewPdf'])->name('entry.Viewpdf');
    Route::get('/entry/pdfUnited/{id}',[App\Http\Controllers\EntryController::class, 'ViewPdfUnitedOther'])->name('entry.ViewPdfUnitedOther');
...
//     Route::post('/other/entry/destroy/{id}','destroy')->name('other.entry.destroy');
    ...
    // PRESERVADO: Rutas de impresión y PDF de otros ingresos
    Route::get('/other/entry/print/{id}',[App\Http\Controllers\OtherEntryController::class, 'print'])->name('other.entry.print');
    Route::get('/other/entry/pdf/{id}',[App\Http\Controllers\OtherEntryController::class, 'ViewPdf'])->name('other.entry.Viewpdf');
...
//     }); 
    ...
    // PRESERVADO: Rutas de PDF y cartera
    Route::get('/purse/pdf/{id}',[App\Http\Controllers\PurseController::class, 'ViewPdf'])->name('purse.Viewpdfc');
...
//     });
    ...
    // PRESERVADO: Ruta de impresión de recibos de terceros
    Route::get('/third/receipts/print/{id}',[App\Http\Controllers\ThirdReceiptsController::class, 'print'])->name('third.receipts.print');
...
//     });
    ...
    // PRESERVADO: Ruta de impresión de recibos de egreso
    Route::get('/egresos/recibos/print/{id}',[App\Http\Controllers\EgresoReceiptController::class, 'print'])->name('egreso.receipts.print');
...
// ============================================================================
// RUTAS DE CONTABILIDAD WEB DESHABILITADAS
// ============================================================================
// Las rutas de vistas de contabilidad han sido deshabilitadas.
// Se preservan las rutas de descarga (Excel) que generan archivos.
// La visualización de datos está disponible a través de /api/v1/accounting/*
// ============================================================================
...
//     Route::get('/contabilidad/informe-mensual','informeMensualView')->name('accounting.informe-mensual');
    ...
    // PRESERVADO: Rutas de descarga (Excel) para contabilidad
    Route::get('/contabilidad/abonos/download',[App\Http\Controllers\AccountingController::class, 'abonosDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.abonos.download');
    Route::get('/contabilidad/otros-ingresos/download',[App\Http\Controllers\AccountingController::class, 'otrosIngresosDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.otros-ingresos.download');
    Route::get('/contabilidad/total-ingresos/download',[App\Http\Controllers\AccountingController::class, 'totalIngresosDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.total-ingresos.download');
    Route::get('/contabilidad/egresos/download',[App\Http\Controllers\AccountingController::class, 'totalEgresosDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.total-egresos.download');
    Route::get('/contabilidad/arqueo-diario/download',[App\Http\Controllers\AccountingController::class, 'arqueoDiarioDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.arqueo-diario.download');
    Route::get('/contabilidad/informe-semanal/download',[App\Http\Controllers\AccountingController::class, 'informeSemanalDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.informe-semanal.download');
    Route::get('/contabilidad/informe-mensual/download',[App\Http\Controllers\AccountingController::class, 'informeMensualDownload'])->middleware(['auth', 'permission:access.accounting'])->name('accounting.informe-mensual.download');
...
// ============================================================================
// RUTAS DE ADMINISTRACIÓN SUPER ADMIN WEB DESHABILITADAS
// ============================================================================
// Herramientas de mantenimiento y base inicial deshabilitadas para la web.
// Toda la gestión administrativa está disponible a través de la API.
// ============================================================================
...
// ============================================================================
// RUTAS DE ADMINISTRACIÓN DE USUARIOS Y ROLES WEB DESHABILITADAS
// ============================================================================
// La gestión de usuarios, roles y permisos ahora se realiza a través de:
// - /api/v1/admin/users/* (para usuarios)
// - /api/v1/admin/roles/* (para roles)
// - /api/v1/admin/permissions/* (para permisos)
// ============================================================================
