    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form method="GET" action="{{ route('accounting.total-ingresos') }}" class="mb-4">
                    <div class="row">
                        <div class="col-md-4">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <a href="{{ route('accounting.total-ingresos.download', ['fecha_inicio' => $fecha_inicio, 'fecha_fin' => $fecha_fin]) }}" 
                ...
                @if(isset($dataset))
                    ...
                    @if(count($dataset['entries']) > 0)
                        <div class="table-responsive" style="max-height: 600px; overflow-y: auto;">
                            <table class="table table-bordered table-striped table-hover" style="margin-bottom: 0; font-size: 0.9em;">
                                <thead class="thead-secondary" style="position: sticky; top: 0; z-index: 10; background-color: #6c757d !important;">
                                    <tr>
                                        <th style="width: 8%;">FECHA</th>
                                        <th style="width: 15%;">ESTUDIANTE/REGISTRO</th>
                                        <th style="width: 12%;">PROGRAMA/TIPO</th>
                                        <th style="width: 10%;">TIPO DE INGRESO</th>
                                        <th style="width: 8%;">TIPO</th>
                                        <th style="width: 12%;">CONCEPTO</th>
                                        <th style="width: 15%;">DESCRIPCIÓN</th>
                                        <th style="width: 8%;">N°RECIBO</th>
                                        <th style="width: 6%;" class="text-right">VALOR</th>
                                        <th style="width: 6%;" class="text-right">SUMA</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($dataset['entries'] as $entry)
                                        <tr>
                                            <td style="padding: 8px;">{{ date('d/m/Y', strtotime($entry['fecha'])) }}</td>
                                            <td style="padding: 8px;">{{ $entry['nombre'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['programa'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['tipo_ingreso'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['tipo'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['concepto'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['descripcion'] }}</td>
                                            <td style="padding: 8px;">{{ $entry['no_recibo'] }}</td>
                                            <td class="text-right" style="padding: 8px;">${{ number_format($entry['valor'], 0, ',', '.') }}</td>
                                            <td class="text-right font-weight-bold" style="padding: 8px;">${{ number_format($entry['suma'], 0, ',', '.') }}</td>
                                        </tr>
                                    @endforeach
                                    <tr style="background-color: #d1e7dd; font-weight: bold; font-size: 1.05em;">
                                        <td colspan="8" class="text-right" style="padding: 12px;">SUMA TOTAL</td>
