    ...
    <div class="table-responsive">
        <div id="example3_wrapper" class="dataTables_wrapper no-footer">
            <table id="example3" class="display dataTable no-footer" style="min-width: 845px" role="grid" aria-describedby="example3_info">
                <thead>
                    <tr>
                        @foreach($head as $item)
                            @if (isset($item['EsVacio']) && $item['EsVacio'])
                ...
                <tbody>
                    @foreach ($thirdEntry as $item)
                        <tr>
                            @foreach($head as $headItem)
                                @if (isset($headItem['EsVacio']) && $headItem['EsVacio'])
                                    <td></td>
                                @elseif (isset($headItem['EsAcciones']) && $headItem['EsAcciones'])
                                    <td>
                                        <a href="{{ route('third.entry.edit', $item->id) }}" class="btn btn-sm btn-warning" title="Modificar">
                                            <i class="fa-solid fa-edit"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteThird({{ $item->id }})" title="Eliminar">
                                            <i class="fa-solid fa-trash"></i>
                                        </button>
                                    </td>
                                @else
                                    @php
                                        $columns = $headItem['Origen'] ?? null;
                                    @endphp
                                    @if($columns)
                                        <td>{{ $item->$columns ?? '-' }}</td>
                                    @else
