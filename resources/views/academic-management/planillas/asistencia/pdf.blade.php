...
<style>
    ...
    * {
    ...
    /* Asegurar márgenes uniformes en toda la página */
    body {
        margin: 0;
        padding: 0;
    }
    ...
    /* Asegurar que el footer de planilla esté visible */
    .footer-planilla {
...
<h2 class="title-planilla">PLANILLA DE ASISTENCIA</h2>
            ...
            @if($estudiantes->count() > 0)
                @foreach($estudiantes as $index => $estudiante)
                    <tr class="no-break">
                        ...
                        <td>{{ $estudiante->telefono_personal ?? '' }}</td>
...
@php
    ...
    $fechaClase = \Carbon\Carbon::parse($fecha_clase);
    $diaSemana = $dias[$fechaClase->dayOfWeek];
    $dia = $fechaClase->day;
    $mes = $meses[$fechaClase->month - 1];
    $anio = $fechaClase->year;
    $fechaFormateada = ucfirst($diaSemana) . ', ' . $dia . ' de ' . ucfirst($mes) . ' de ' . $anio;
@endphp
...
<div class="footer-planilla">
    ...
    <div class="footer-bottom" style="width: 100%; margin-top: 8px; font-size: 12px; color: #000;">
        <div class="date" style="float: left; display: block; color: #000; font-size: 12px;">{{ $fechaFormateada }}</div>
